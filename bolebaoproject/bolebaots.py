...
#     #get_url是通过XS单号对SN表进行查询，获取出id和update_by
#     get_url = 'https://crm.meiqia.com/api/v1.0/one/dsl/query?dsl={"SN":{"fields":["id","ddatetime"],"cond":{"==":{"name":"' + parse.quote(
#         date['name']) + '"}}}}'
#     obj_id = Method(get_url).get_id()
#     if obj_id :
#         '''如果有这条记录，通过对比时间来进行操作，时间是最新的更新到记录中'''
#
#         if date['ddatetime'] > obj_id[0]['ddatetime']:
#
#             put_url = '/api/v1.0/one/SN/'+obj_id[0]['id']
#             date['version'] = obj_id[0]['version']
#
#             Method(put_url,date).put_date()
#         else:
#             print("kkk")
#
#     else:
#         url = "https://crm.meiqia.com/api/v1.0/one/SN/"
#         Method(url,{"objects":[date]}).post_date()
#期初同步webservice中的数据
def query_insert(date):
	global sn
	date_sn = date['name']
