...
from analysis import Information
    ...
    def request(self, url):
        ...
        if result :
            if result['errcode'] == '0':
                ...
                loger().info(result,'有错误，错误的url为：',url)
                return "error"

            else:
                loger().info(result,url)
                return "error"
        else:
            return None
    ...
    def run(self):
        ...
        while True:
            ...
            if key == 'customer':# 购买渠道
                ...
                Information(value).customer()

            elif key == 'saleorderlist':  # 订单信息

                Information(value).saleorderlist()
                ...
                Information(value).person()

            elif key == 'eb_tradelist':

                Information(value).eb_tradelist()

            elif key == 'saleoutlistall':

                Information(value).saleoutlistall()
...
if __name__ == '__main__':
    ...
    end_url = 'from_account=bolebaokaifa&to_account=bolebao&app_key=opa87ad77a2a44f12df&token=' + \
              token + '&page_index=' + page_index + '&rows_per_page=1000&ds_sequence=1'
