...
def data_query(data, data_name, url, alias):
    red = data
    data_url = '/api/v1.0/one/dsl/query?dsl={"' + data_name + '":{"fileds":"id","cond":{"==":{"name":"' + red[
        alias] + '"}}}}'
    ...
    if red_id:
        ...
        reds = red.pop('code')
        reds['version'] = red_id[0]['version']
        put_meiqia(reds, red_url)
    else:
        url = url
...
def revise_data(re_data):
    revise_data = re_data
    revise_url = '/api/v1.0/one/dsl/query?dsl={"Tickets_ReturnGoods":{"fileds":"id","cond":{"==":{"XSNumbers":"' + \
                 revise_data['code'] + '"}}}}'
...
def get_id(object_name, record):
    get_url = '/api/v1.0/one/dsl/query?dsl={"' + object_name + '":{"fileds":"id","cond":{"==":{"name":"' + parse.quote(
        record) + '"}}}}'
            ...
            if key == 'customer':  # 购买渠道
                ...
                for i in data:
                    ...
                    if 'mobile' in i or 'phone' in i:
                        if 'mobile' in i and 'phone' in i:
                            ...
                            if i['phone']:
                                phone(dict_c, i['phone'])
                    data_query(dict_c, "Channel", url, 'name')
                ...
                for i in data:
                    ...
                    if 'fRetailPrice' in i:
                        dict_i['TaxUnitPrice'] = i['fRetailPrice']  # 含税单价
                ...
                for i in data:
                    ...
                    data_query(list_p, "Salesman", url, 'PersonnelNumber')
                ...
                for i in data:
                    ...
                    if 'cshipcode' in i:
                        if i['cshipcode']:
                            eb_data = data_query(i['cshipcode'])
                            if eb_data:
                                list_e['name'] = eb_data['id']
